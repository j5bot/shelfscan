...
export function Scanlist(props: ScanlistProps) {
    ...
    return (<ul className="flex items-center gap-3">{codes.map(code => {
        ...
        const { name = 'Nothing Found', versions = [], thumbnail_url: thumbnailUrl } = bggInfo?.[0] ?? {};

        return <li className="rounded-md bg-orange-100 p-4">
            <p>{name}</p>
            <p className="text-sm text-gray-400">[{code}]</p>
            <p><img src={thumbnailUrl} alt={name} /></p>
        </li>;
