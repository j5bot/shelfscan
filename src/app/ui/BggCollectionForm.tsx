...
export const BggCollectionForm = ()=> {
    const [state, formAction, isPending] = useActionStateWithCallbacks<string>({
        action: bggGetCollection as
            Parameters<(typeof useActionStateWithCallbacks<string>)>[0]['action'],
        initialState: '',
        callbacks: [
            (state: string) => {
                const rawItems = getPageDOM(state, true).getElementsByTagName('items')[0];
